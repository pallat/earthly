...
update-buildkit:
    ...
    ARG BUILDKIT_GIT_SHA=earthly-main
    BUILD ./buildkitd+update-buildkit --BUILDKIT_GIT_SHA=$BUILDKIT_GIT_SHA
    RUN --no-cache go mod edit -replace "github.com/moby/buildkit=github.com/earthly/buildkit@$BUILDKIT_GIT_SHA"
    RUN --no-cache go mod tidy
    RUN --no-cache echo -e "\n\n\nDONT FORGET TO mv go.mod-fixme go.mod && mv go.sum-fixme go.sum\n\n\n\n"
